...
def step_decay(epoch, init_learning_rate=0.0001, decay_rate=0.25, epochs_drop=10.0):
	...
	lrate = init_learning_rate * math.pow(decay_rate, math.floor((epoch)/epochs_drop))
	return lrate
...
train_datagen.fit(train_T2)

val_datagen = ImageDataGenerator()

# transform the validation data with statistics from train
# val_T2 -= train_datagen.mean
# val_T2 /= train_datagen.std
...
os.chdir(output_dir)
np.save('T2_history_conv2', history_transfer.history)
model.save('T2_model_conv2.h5')
...
#predict
score = model.evaluate(test_T2,test_1p19q,batch_size=64)
